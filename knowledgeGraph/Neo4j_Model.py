...
class Neo4j_Handle:
    ...
    def connectNeo4j(self):
        self.graph = Graph("http://127.0.0.1:7474", username="neo4j", password="ztj19991124")
        self.matcher = NodeMatcher(self.graph)

    # 一.实体查询
    def get_entity_info(self, name: object) -> list:
        """
        查找该entity所有的直接关系
        :param name:
        :return:
    ...
    def actor_category_movie(self, name, category):
        answer = self.graph.run(
            "MATCH (m:Movie)-[rel:ACTOR]->(p:Person) where p.name='" + name
            + "' and m.category=~'.*" + category + ".*' return rel").data()
    ...
    def actor_movie_category(self, name):
        answer = self.graph.run(
            "match (m:Movie)-[:ACTOR]->(p:Person{name:$name}) return p.name as name, m.category as category",
            name=name).data()
    ...
    def actor_actor_movie(self, name1, name2):
        answer = self.graph.run(
            "match (p:Person{name:$pname})<-[rel1:ACTOR]-(m:Movie)-[rel2:ACTOR]->(other:Person{name:$oname}) return "
            "rel1, rel2",
            pname=name1,
            oname=name2).data()
    ...
    def gt_rate_category_movie(self, rate, category):
        answer = self.graph.run(
            "MATCH (m:Movie) where m.rate>$rate and m.category =~'.*category.*' return m.name as name, m.rate as "
            "rate, m.category as category",
            rate=rate, category=category).data()
